...
export default {
  ...
  methods: {
    ...
    checkPermission(api, method) {
      if (api) {
        if (this.users.node === undefined) {
          return true
        }
        const apis = this.users.node
        const rest = apis.filter(function(item) {
          return item[1] === api && (item[2] === method || item[2] === '*')
        })
    ...
    filterEmpty(params) {
      for (const key in params) {
        ...
        const isEmpty = type === 'undefined' || type === null
        if (isEmpty) {
          delete params[key]
        } else {
